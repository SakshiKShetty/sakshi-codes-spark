...
export function Nav() {
  ...
  const renderNavLink = (hash: string, label: string, className: string, onClick?: () => void) => {
    if (onHome) {
      return (
        <a key={hash} href={`#${hash}`} onClick={onClick} className={className}>{label}</a>
      );
    }
    return (
      <Link key={hash} to="/" hash={hash} onClick={onClick} className={className}>{label}</Link>
    );
  };
      ...
      <div className="mx-auto max-w-6xl px-4">
        <nav className={`glass-strong rounded-2xl px-4 sm:px-6 py-3 flex items-center justify-between ${scrolled ? "glow-ring" : ""}`}>
          ...
          <div className="hidden md:flex items-center gap-6 text-sm">
            {links.map(l => renderNavLink(l.hash, l.label, "text-muted-foreground hover:text-foreground transition-colors"))}
            <Link to="/resume" className="text-muted-foreground hover:text-foreground transition-colors">Resume</Link>
            {renderNavLink("contact", "Let's talk", "rounded-full px-4 py-1.5 bg-gradient-to-r from-primary to-accent text-primary-foreground font-medium hover:opacity-90 transition")}
          </div>
        ...
        {open && (
          <div className="md:hidden mt-2 glass-strong rounded-2xl p-4 flex flex-col gap-3 text-sm">
            {links.map(l => renderNavLink(l.hash, l.label, "text-muted-foreground hover:text-foreground", () => setOpen(false)))}
